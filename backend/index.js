...
const fetchCoinsData = async pageIndex => {
    try {
        const result = await phinGet(`https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=${pageSize}&page=${pageIndex}&sparkline=false`);
        console.log(result.body.map(coin => ({
            id: coin.id,
            name: coin.name,
            image: coin.image,
